...
// Get S3 presigned upload for promotion banner upload
export const getS3SignatureForPromotion = async (req: Request, res: Response) => {
    try {
        ...
        console.error('Get promotion S3 signature error:', error);
        return sendError(res, error.message || 'Failed to get upload signature', 500);
    }
};

// Upload promotion banner via backend (CORS fallback)
export const uploadPromotionImageViaBackend = async (req: Request, res: Response) => {
    try {
        ...
        console.error('Upload promotion image via backend error:', error);
        return sendError(res, error.message || 'Failed to upload promotion image', 500);
    }
};
